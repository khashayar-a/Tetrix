...
init() ->
    ...
    i2c_sup:start_link([{?I2C_CHANNEL, "/dev/i2c-1"}]),
    %% set pin directions
    i2c:write(?I2C_CHANNEL, ?ADDR, {?IODIR, 16#f}),
    %% reset pin value
    i2c:write(?I2C_CHANNEL, ?ADDR, {?GPIO, 0}).
...
start(Count) when Count < 16 ->
    %% turn on the led
    i2c:write(?I2C_CHANNEL, ?ADDR, {?GPIO, Count bsl 4}),
    timer:sleep(1000),
    
    %% read DIP switches
    i2c:write(?I2C_CHANNEL, ?ADDR, {?GPIO}),
    timer:sleep(10),
    {Val}=i2c:read(?I2C_CHANNEL, ?ADDR, 1),
