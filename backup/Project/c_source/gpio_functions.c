...
{
  double time, speed;
  //time = (double) difftime(end, start);
  //speed = (total_movement * 0.00525) / ( time / 1000000); 
...
void* loop_retrieving(void *arg)
{
  ...
  //int *originalPins = malloc(sizeof(pins));
  ...
  while(1)
  {
    ...
    //memcpy(originalPins, pins, sizeof(pins));
    ...
    if(is_movement( ))
    {
      ...
      int pre_movement =  calculate_movement(pins_state(pins),pins_state(pinsNew));
      if(pre_movement != 5)
          //movement += calculate_movement(pins_state(pins), pins_state(pinsNew));
          movement += pre_movement;
          total_movement += pre_movement;

      if(movement > 1 && (pre_movement != 1) )
      {
        /* printf("**movement is %d, before: (%d, %d, %d), new_input: (%d, %d, %d)\n",  */
        /*       movement, pins[0], pins[1], pins[2], pinsNew[0], pinsNew[1], pinsNew[2]); */
      }
...
double diffclock(clock_t clock1,clock_t clock2) { double
diffticks=clock1-clock2; double diffms=(diffticks*1000)/CLOCKS_PER_SEC; return
diffms; }
