...
%%    P = pcbnif:start_pcb(),
    {nodePcb, 'node3@odroidx2' } ! [speed,50],
%%    pcbnif:set_speed(20,P),
    timer:sleep(5000),
    calculate(0),
    ok.
    ...
    case map_gen:node_ahead(Car_Position, Car_Heading) of
	    ...
	    {nodePcb, 'node3@odroidx2'} ! [dir,4];
      	[P1,P2,P3] ->
     		...
     		{true, true} ->
		    Third = output_steering(Angle);
      		    %%Third = round(Angle * 2.3) + 90;
     		{true,false} ->
		    Third = output_steering(-25);
...
%%      	    io:format("DRIVING WITH ~p ~n" , [Third]),
            {nodePcb, 'node3@odroidx2'} ! [angle,Third],
            {nodePcb, 'node3@odroidx2'} ! [dir,1]
...
output_steering(Steering_ang) ->
    round((2.3*Steering_ang)+91.14286).
%%    round((1.95483*Steering_ang)+91.14286).
