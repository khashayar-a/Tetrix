...
get_opposite_nodes_ahead([Head| T], Buff) ->
    Dash = ets_lookup(Head),
    {ok,[OP]} = offsetCalculation:calculate_offset_list(?InputLaneD, ?LaneOpposite, 
							Dash#dash_line.points),   
    get_opposite_nodes_ahead(T, Buff ++ [OP]);
get_opposite_nodes_ahead([], Buff) ->
    Buff.
